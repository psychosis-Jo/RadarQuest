...
**4 步 funnel**：捕捉 → 决定留 → 去做 → 沉淀，对应 4 个 nav tab：
...
│  右上：N 颗 · 总 XP                           │ <lg 隐藏  │
└──────────────────────────────────────────────┴───────────┘
       移动端：底 tab bar = 捕捉 / 星云 / 我的 / 设置
```

**主视图 = 你的星云**（满屏力导向画布，不是列表）

**4 步 funnel**（详见 IA.md §1）：
...
- 抓来 → `items.state = 'unprocessed'` → 出现在 `捕捉`（按 topic 分组）
- 在 `捕捉` ✅ 保留 → `state = 'kept'` → 卡片沉到"已保留"区 + 出现在 `星云`
- 在 `捕捉` ⭐ 收藏 → `state = 'kept + saved'` → 立即从捕捉消失 + 出现在 `星云`（带收藏角标）
- 在 `捕捉` ❌ 忽略 → `state = 'dismissed'` → 30 天后硬删；30 天内若再次 spike 自动召回
- 点击 `星云` 上的星 → Drawer → 5 动作 → 推进任务 / 技能 / 星座 → 沉淀到 `我的`
...
## 8. 捕捉 · 主题聚合卡片流（`/capture`）

> ⏳ 规划中（页面未实现）。原 5-Tab 概念已废弃，本节是替代设计。
> 数据源 / 卡片视觉 / Triage 交互见 [IA.md §4](IA.md)，本节只列视觉骨架。
...
**Topic 排序**：按数量降序——AI 12 > 一人公司 5 > 自我管理 8 > 未分类 3，
最多的话题排最上。

**Card = 复用 §9 的 Item Card**（不是星云元素），底部加 Triage 行：
...
│  [✅ 保留] [⭐ 收藏] [❌ 忽略]                   │  ← Triage quick action
└─────────────────────────────────────────────────┘
```

**Triage 行为**（数据状态 + UI 反馈）：

| 动作 | state | UI 反馈 | 后续 |
|------|-------|---------|------|
| ✅ 保留 | `kept` | 卡片沉到"已保留"区（带"已保留"角标），原位置消失 | item 出现在星云 |
| ⭐ 收藏 | `kept + saved` | 立即从捕捉页消失 | item 出现在星云 + 收藏角标 |
| ❌ 忽略 | `dismissed` | 卡片淡出消失 | 30 天后硬删；30 天内若再次 spike 自动召回（DESIGN §5.3） |

> **关键**：保留后**不立刻从捕捉页消失**（避免无反馈），而是**沉到"已保留"区**。
> 这是 funnel 可视化——用户能看到"今天留了 7 条"。

**空状态**：
- 0 条新信息：`"今晚海面平静，暂无新信息"`
- 全部已处理：所有 topic section 折叠，只剩"已保留"区
...
完整 IA 文档：[IA.md](IA.md)。本节只列路由对照，**改任何路由前先看 IA.md**。
...
```
抓取 → items.state = 'unprocessed'
        ↓ /capture 展示
    保留 → items.state = 'kept'         → 出现在 /
    收藏 → items.state = 'kept + saved' → 出现在 /
    忽略 → items.state = 'dismissed'    → 30 天后硬删
        ↓
...
> **本节是路由规范。** 改路由 / 加页面 / 改 nav 顺序都先更新 IA.md，再来这里同步。
